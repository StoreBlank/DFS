...
class SawyerCoffeeButtonEnvV2(SawyerXYZEnv):
    ...
    def _get_quat_objects(self):
        return self.model.body('coffee_machine').quat
    ...
    def reset_model(self):
        ...
        self.model.body('coffee_machine').quat = quaternion

        pos_mug = self.obj_init_pos + rotate_vector_by_quaternion(np.array([0.0, -0.22, 0.0]), quaternion)
        self._set_obj_xyz(pos_mug)

        pos_button = self.obj_init_pos + rotate_vector_by_quaternion(np.array([0.0, -0.22, 0.3]), quaternion)
        self._target_pos = pos_button + rotate_vector_by_quaternion(np.array([0.0, self.max_dist, 0.0]), quaternion)
