...
class SawyerDrawerOpenEnvV2(SawyerXYZEnv):
    def __init__(self, tasks=None, render_mode=None, random_level=1):
        ...
        # self._last_rand_vec = np.array([-0.4, 0.6, 0.0])
    ...
    def _get_pos_objects(self):
        return self.get_body_com("drawer_link") + rotate_vector_by_quaternion(np.array([0.0, -0.16, 0.0]), self.data.body("drawer_link").xquat)
    ...
    def reset_model(self):
        ...
        self.model.body('drawer').quat = quaternion

        # Set _target_pos to current drawer position (closed) minus an offset
        self._target_pos = self.obj_init_pos + rotate_vector_by_quaternion(np.array(
