...
class SawyerHammerEnvV2(SawyerXYZEnv):
    ...
    def __init__(self, tasks=None, render_mode=None, random_level=1):
        ...
        if random_level == 2:
            obj_low = (-0.2, 0.4, 0.0)
            obj_high = (0.2, 0.55, 0.0)
        if random_level >= 3:
            obj_low = (-0.4, 0.4, 0.0)
            obj_high = (0.4, 0.55, 0.0)
        ...
        if random_level <= 3:
            ...
            self._random_reset_space = Box(
                np.array(obj_low + (-0.15,)), np.array(obj_high + (0.15,))
            )
    ...
    def reset_model(self):
        ...
        if self.random_level == 4:
            z = self.hammer_init_pos[-1]
            self.hammer_init_pos = self.hammer_init_pos[:-1]
        w = (1 - z ** 2) ** 0.5
        quaternion = np.array([w, 0.0, 0.0, z])

        self.nail_init_pos = self._get_site_pos("nailHead")
        self.obj_init_pos = self.hammer_init_pos.copy()
        self.model.body('hammer').quat = quaternion
