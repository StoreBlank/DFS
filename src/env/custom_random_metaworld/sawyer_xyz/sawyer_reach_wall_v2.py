...
class SawyerReachWallEnvV2(SawyerXYZEnv):
    ...
    def __init__(self, tasks=None, render_mode=None, random_level=1):
        ...
        if random_level <= 3:
            self._random_reset_space = Box(
                np.hstack((wall_delta_low, goal_low)),
                np.hstack((wall_delta_high, goal_high)),
            )
        else:
            self._random_reset_space = Box(
        ...
        # self._last_rand_vec = np.array([-0.4, 0.6, 0.015, -0.05, 0.85, 0.05])
    ...
    def _get_pos_objects(self):
        return self.get_body_com("wall")

    def _get_quat_objects(self):
        return self.model.body('wall').quat
