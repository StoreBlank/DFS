...
class SawyerWindowCloseEnvV2(SawyerXYZEnv):
    ...
    def __init__(self, tasks=None, render_mode=None, random_level=1):
        ...
        if random_level == 0:
            obj_low = (0.0, 0.85, 0.2)
            obj_high = (0.0, 0.9, 0.2)
        if random_level == 1:
            obj_low = (-0.05, 0.8, 0.2)
            obj_high = (0.05, 0.9, 0.25)
        if random_level == 2:
            obj_low = (-0.1, 0.75, 0.2)
            obj_high = (0.1, 0.9, 0.3)
        if random_level >= 3:
            obj_low = (-0.2, 0.7, 0.2)
            obj_high = (0.2, 0.9, 0.35)
    ...
    def reset_model(self):
        ...
        self.window_handle_pos_init = self._get_pos_objects() + np.array(
            rotate_vector_by_quaternion([move, 0.0, 0.0], quaternion)
        )
        self.data.joint("window_slide").qpos = move
