...
def evaluate(env_id, task_id, agent, video, num_episodes, video_name, env_config):
...
@hydra.main(version_base=None, config_path="../configs", config_name="eval_multitask_config")
def main(cfg):
    algos = cfg.algos
    ...
    df = pd.DataFrame(columns=env_config.env_ids, index=list(algos.keys()))
    ...
    for task_id, env_id in enumerate(env_ids):
        ...
        for algo, path in algos.items():
            ...
            video_name = f"{algo}_{env_id}"
            # reward, _ = evaluate(env, task_id, agent, video, eval_episodes, video_name)
            # print(f"Reward: {reward}")
            # df.loc[algo, env_id] = reward
            _, success_rate = evaluate(env_id, task_id, agent, video, eval_episodes, video_name, env_config)
            print(f"Success rate: {success_rate}")
            df.loc[algo, env_id] = success_rate
